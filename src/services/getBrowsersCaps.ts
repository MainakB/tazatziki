...
export const multiCapabilities = async () => {
  ...
  if (browser.indexOf(',') > -1 || (browser.startsWith('[') && browser.endsWith(']'))) {
  ...
  // }
  browserCapsList = await Promise.all(valSent.map(async browserVal => getCaps(browserVal))).then(caps => caps);
