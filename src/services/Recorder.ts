...
//       `${fullname.replace(/\s+/g, "-")}--${browserName}--${timestamp}`
//     )
//       .replace(/%../g, "")
//       .replace(/\./g, "-")
//       .replace(/[/\\?%*:'|"<>()]/g, "");
...
//       filename =
//         filename.slice(0, truncLength) + "--" + filename.slice(-truncLength);
//     }
...
//       "yuv420p",
//       "-vf",
//       `"scale=1200:trunc(ow/a/2)*2","setpts=${config.videoSlowdownMultiplier}.0*PTS"`,
//       `"${videoPath}"`,
//     ];
...
//     const promise = Promise.all(this.screenshotPromises || []).then(
//       () =>
//         new Promise((resolve) => {
...
//     do {
//       this.sleep(100);
//       let currentSizes = videos.map((filename) => ({
//         filename,
...
//                   ...accFilter,
//                   curFilter
//                     .filter((v) => v.filename === curOuter.filename)
//                     .pop(),
//                 ],
//                 []
//               )
//               .map((v) => v.size)
//               .reduce(
//                 (accInner, curInner) => accInner && curInner === curOuter.size,
//                 true
//               ),
//           true
//         );
...
//           Object.keys(videoReporterObject.capabilities)[0]
//         ]
//       : videoReporterObject.capabilities;
...
//    */
//   onSuiteStart(suite) {
//     this.testnameStructure.push(
//       suite.title.replace(/ /g, "-").replace(/-{2,}/g, "-")
//     );
//   },
...
//     const fullname = this.testnameStructure
//       .slice(1)
...
//     this.frameNr = 0;
//     this.recordingPath = path.resolve(
//       config.outputDir,
//       config.rawPath,
//       this.testname
//     );
...
//           this.recordingPath,
//           this.frameNr.toString().padStart(4, "0") + ".png"
//         );
//         try {
...
//       config.outputDir = options.logFile
...
//     config.videoSlowdownMultiplier =
//       options.videoSlowdownMultiplier || config.videoSlowdownMultiplier;
//     config.videoRenderTimeout =
//       options.videoRenderTimeout || config.videoRenderTimeout;
...
//     config.maxTestNameCharacters =
//       options.maxTestNameCharacters || config.maxTestNameCharacters;
...
//     const allureConfig = runnerInstance.reporters
//       .filter((r) => r === "allure" || r[0] === "allure")
//       .pop();
...
//     config.debugMode =
//       logLevel.toLowerCase() === "trace" || logLevel.toLowerCase() === "debug";

//     helpers.debugLog(
//       "Using reporter config:" +
//         JSON.stringify(runnerInstance.reporters, undefined, 2) +
//         "\n\n"
//     );
//     helpers.debugLog(
//       "Using config:" + JSON.stringify(config, undefined, 2) + "\n\n\n"
//     );

//     // Jasmine and Mocha ought to behave the same regarding test-structure
//     this.framework =
//       runnerInstance.framework === "cucumber"
//         ? cucumberFramework
//         : defaultFramework;
...
//     const command =
//       jsonWireMsg.endpoint && jsonWireMsg.endpoint.match(/[^\/]+$/);
//     const commandName = command ? command[0] : "undefined";

//     helpers.debugLog(
//       "Incoming command: " +
//         jsonWireMsg.endpoint +
//         " => [" +
//         commandName +
//         "]\n"
//     );
...
//     if (
//       (!config.recordAllActions &&
//         (config.excludedActions.includes(commandName) ||
//           !config.jsonWireActions.includes(commandName))) ||
//       !this.recordingPath
//     ) {
//       return;
//     }
...
//     if (!fs.existsSync(this.recordingPath)) {
//       helpers.debugLog(
//         `Creating: ${this.recordingPath}, as it not exists...\n`
//       );
...
//     if (
//       test.state === "failed" ||
//       (test.state === "passed" && config.saveAllVideos)
//     ) {
//       const filePath = path.resolve(
//         this.recordingPath,
//         this.frameNr.toString().padStart(4, "0") + ".png"
...
//     Promise.all(this.videoPromises).then(wrapItUp).catch(wrapItUp);

//     abortTimer = setTimeout(() => {
//       this.write(
//         `videoRenderTimeout triggered before ffmpeg had a chance to wrap up`
//       );
//       wrapItUp();
...
//       console.log(
//         `videoRenderTimeout triggered, not all videos finished writing to disk before patching Allure`
//       );
//     }

//     fs.readdirSync(config.allureOutputDir)
...
//       .filter((allureFile) =>
//         this.videos.includes(fs.readFileSync(allureFile).toString())
